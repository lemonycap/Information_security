...
public class Steganography {
    ...
    public BufferedImage encode(final BufferedImage bufferedImage, final String message,
                                final int offset) throws SteganographyException {
        return this.addText(bufferedImage, message, offset);
    }


    /**
     * Decodes and gets a hidden message from the given image. The used algorithm is
    ...
    public String decode(final BufferedImage bufferedImage, final int startingOffset)
        ...
        try {
            ...
            throw new SteganographyException(Consts.DECODING_ERROR_GENERAL);
        }
    }
    ...
    private BufferedImage addText(BufferedImage bufferedImage, final String text, final int offset)
        ...
        try {
            ...
            // Encode the message
            bufferedImage = this
                    .encodeText(bufferedImage, msg, offset + Consts.HIDDEN_MESSAGE_BIT_LENGTH);
        } catch (Exception e) {
            throw new SteganographyException(Consts.ENCODING_ERROR_GENERAL, e);
        }
    ...
    private BufferedImage encodeText(final BufferedImage bufferedImage, final byte[] addition,
        ...
        if ((width * height) >= (addition.length * 8 + offset)) {
            ...
            throw new SteganographyException(Consts.ENCODING_ERROR_BIG_MESSAGE);
        }
        return bufferedImage;
    }
    ...
    private byte[] decodeText(final BufferedImage bufferedImage, final int startingOffset) {
        ...
        final int height = bufferedImage.getHeight();
        final int offset = startingOffset + Consts.HIDDEN_MESSAGE_BIT_LENGTH;
        int length = 0;
